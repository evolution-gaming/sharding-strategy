 ...
 *   [[https://github.com/akka/akka/blob/main/akka-cluster-sharding/src/main/scala/akka/cluster/sharding/ShardCoordinator.scala]]
 */
trait ShardingStrategy[F[_]] {
   ...
   *
   * @param requester
   *   actor reference to the [[akka.cluster.sharding.ShardRegion]] that requested the location of
   *   the shard, can be returned if preference should be given to the node where the shard was
   *   first accessed
   * @param shard
